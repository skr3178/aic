#
#  PerceptionInsertSFPDriveSeatV3 — fix the CONTACT PREDICATE, then instrument. Nothing else changes.
#
...
SEAT_TIP = 0.005      # plug tip within 5mm of (or below) the port datum => IT IS IN.
                      # On the cage face the tip sits at +42..46mm, so this is unambiguous.
Z_FLOOR = -0.015
STEP = 0.0005
DT = 0.05
LOG = "/home/skr/aic_data/seat_v3_log.jsonl"


class PerceptionInsertSFPDriveSeatV3(PerceptionInsertSFPDrive):
    ...
    def insert_cable(self, task, get_observation, move_robot, send_feedback):
        ...
        while z >= Z_FLOOR:
            ...
            try:
                ...
                if tip_above < SEAT_TIP:
                    self.get_logger().info(
                        f"*** SEATED (measured tip {tip_above*1000:+.1f}mm vs port) at spiral {spiral_k}, "
                    ...
                    self._rec(trial=self._trial_idx, port=self._task.port_name, step=step, event="SEATED",
                              tip_above_port_mm=round(tip_above * 1000, 1), spiral_k=spiral_k,
