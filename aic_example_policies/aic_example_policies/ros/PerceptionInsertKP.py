...
USE_GT_PORT = False
USE_GT_PORT_POS = True    # diagnostic: GT port POSITION only (isolate the board-yaw ORIENTATION fix)
...
SWEEP_BOARD = True
SWEEP_ONLY = True        # diagnostic: do the sweep, save the point cloud, skip insertion
...
class PerceptionInsertKP(Policy):
    def __init__(self, parent_node):
        ...
        self.get_logger().info(
            f"PerceptionInsertKP: DualPoseNet {DUAL_CKPT} (img {self._img_size}, vocab {len(self._vocab)}) "
    ...
    def _perceive(self, get_observation):
        ...
        kp_batch = torch.stack([self._kp_tensor(obs.left_image),
                                self._kp_tensor(obs.center_image),
                                self._kp_tensor(obs.right_image)], 0).to(self._device)
        with torch.no_grad():
            uv = self._kp(kp_batch).float().cpu().numpy()      # (3,2) pixels in the 288x256 image
