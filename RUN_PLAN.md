...
| M8 | Iterate by result | ⏳ **→ M6d is the branch taken** | M7a says perception (69–220 mm on eval) is the sole remaining bottleneck: finish the stopped retrain on **merged v1-native + v2**, then heatmap head if in-dist close-range error stays high. Reactive stack needs no work — do not tune search params until perception lands <30 mm |
...
|---|---|---|
| Semantic anchor | DualPoseNet (12.1 M): 3 wrist imgs → port+plug pose, camera frame | ✅ built · ⏳ v2 retrain finishing |
